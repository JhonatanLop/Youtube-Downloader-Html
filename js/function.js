...
// adiciona a função downloadVideo ao evento de clique do botão de download
botao_download.addEventListener("click", downloadVideo);

// parte download
// cria a função downloadVideo
async function downloadVideo() {
    ...
    const response = await fetch("http://localhost:5500/python/download", {
      method: "POST",
      body: formData
    });
    ...
    if (response.status === 200) {
      ...
      alert("Ocorreu um erro ao fazer o download, por favor tente novamente.");
    }
}
